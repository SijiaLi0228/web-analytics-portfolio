...
The selected public dataset is not a Matomo export, but it is useful because it has realistic product-level user behavior. The same modeling approach can be applied to Matomo events once tracking is implemented.

## 1. Tracking Objective

The tracking setup should help answer these business questions:

- How many users view products, add items to cart, remove items from cart, and purchase?
- Which categories and brands produce the strongest conversion?
...
| Journey step | Matomo event category | Event action | Normalized event name | Required properties |
|---|---|---|---|---|
| Product page viewed | Ecommerce | View Item | `view_item` | `product_id`, `category`, `brand`, `price`, `session_id` |
| Add to cart | Ecommerce | Add To Cart | `add_to_cart` | `product_id`, `quantity`, `session_id` |
| Remove from cart | Ecommerce | Remove From Cart | `remove_from_cart` | `product_id`, `quantity`, `session_id` |
| Checkout started | Ecommerce | Begin Checkout | `begin_checkout` | `cart_value`, `session_id` |
...
- Is the issue tracking quality, product interest, cart friction, or purchase conversion?
- Which category or product group should we investigate first?
